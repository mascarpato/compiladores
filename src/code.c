...
TAC* geraCod_noLiteral(comp_tree_t *node)
{
	...
	tac->op1 = node->data.symbol->value;
	tac->prev = NULL;
	tac->next = NULL;
...
TAC* geraCod_noIdent(node)
{
	TAC *tac1 = malloc(sizeof(TAC));
	tac1->type = TAC_LOADI;
	tac1->res = node->data.local;
	tac1->op1 = node->data.symbol->symAddr;
...
TAC* generateCode(comp_tree_t *root)
{
  ...
  // TODO
	switch(node->data.type)
	{
		...
		case IKS_AST_IDENTIFICADOR:
			node.code = geraCod_noIdent(node);
		case IKS_AST_LITERAL:
			node.code = geraCod_noLiteral(node);
